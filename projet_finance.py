...
# Fonction pour télécharger les données historiques de l'action
def download_stock_data(ticker, start_date, end_date):
    stock = yf.Ticker(ticker)
    data = stock.history(start=start_date, end=end_date)
    return data['Close']

# Fonction pour effectuer la régression linéaire et prédire la valeur future
def perform_linear_regression(stock_data):
    ...
    # Régression Ridge avec un alpha par défaut
    ridge = Ridge()
    ridge.fit(X_scaled, y)

    # Prédiction sur la dernière observation
    last_observation = X[-1].reshape(1, -1)
    predicted_price = ridge.predict(scaler.transform(last_observation))[0][0]

    # Plot des résultats
    plt.figure(figsize=(10, 6))
    plt.scatter(X, y, color='blue', label='Données historiques')
    plt.plot(X, ridge.predict(X_scaled), color='red', linestyle='--', label='Régression linéaire')
    plt.xlabel('Jours')
    plt.ylabel('Prix de l\'action')
    plt.title('Régression linéaire pour prédire le prix de l\'action')
    plt.legend()
    plt.show()
...
# Paramètres
ticker = 'BFH'  # Ticker de l'action (Apple dans cet exemple)
start_date = '2024-01-01'  # Date de début des données historiques
...
predicted_price = perform_linear_regression(stock_data)

print("La valeur future prédite de l'action {} est : {:.2f}".format(ticker, predicted_price))